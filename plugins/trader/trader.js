...
Trader.prototype.writeCacheTrader = function(type,price,amount){
    ...
    */
    ...
    if (fsw.existsSync(filecache)) {
        ...
        if(data.length > 10){
          defaultData = data.replace(new RegExp('undefined','g'),'false');
        }else{
    ...
    if(type === "buy"){
      ...
      makeJson = '{"asset":'+this.portfolio.asset+',"currency" : '+this.portfolio.currency+', "amount" : '+this.portfolio.asset+', "buyPrice" : '+(this.portfolio.asset < 0.0001 ? 0 : readCache.buyPrice)+', "sellPrice" : '+(this.portfolio.asset > 0.001 ? 0 : readCache.sellPrice)+', "stopbuy" : '+readCache.stopbuy+', "stopsell" : '+readCache.stopsell+'}';
